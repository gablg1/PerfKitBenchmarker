...
class BaseLinuxMixin(virtual_machine.BaseOsMixin):
  ...
  def SetupHostFirewall(self):
    ...
    if hasattr(self, 'allowed_ip_range') and self.allowed_ip_range:
      self.RemoteHostCommand('sudo iptables -A '
                             'INPUT --source %s -p tcp -j ACCEPT' % self.allowed_ip_range)
      self.RemoteHostCommand('sudo iptables -A '
                             'INPUT --source %s -p udp -j ACCEPT' % self.allowed_ip_range)
      self.RemoteHostCommand('sudo iptables -A '
                             'OUTPUT --destination %s -p udp -j ACCEPT' % self.allowed_ip_range)
      self.RemoteHostCommand('sudo iptables -A '
                             'OUTPUT --destination %s -p tcp -j ACCEPT' % self.allowed_ip_range)
